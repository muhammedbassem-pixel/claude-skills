...
| [`iac-scan`](iac-scan/) | IaC & K8s manifest misconfig scan (Terraform/CFN/ARM/Bicep/Dockerfile/Serverless/K8s/Helm) | Checkov + Trivy config |
...
scan), per scanner or workload (k8s), per surface or package (threat hunting), per module or severity (SCA), per IaC stack (iac-scan) — then aggregate
...
- Reports are written outside any scanned repo (under `~/gitleaks/`, `~/aws-audit/`,
  ...
  `~/threat-hunting/`, `~/sca/`, `~/iac-scan/`) so unredacted secrets and findings can't be committed by accident. Treat
